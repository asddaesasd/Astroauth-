...
function initializeDefaultApp() {
    appsRef.once('value', (snapshot) => {
        if (!snapshot.exists()) {
            console.log('No applications found. Creating default application...');
            const defaultApp = {
                name: 'default',
                isActive: true,
                createdAt: Date.now()
            };
            appsRef.child(defaultApp.name).set(defaultApp);
            console.log('Default application created:', defaultApp);
        }
    });
}
